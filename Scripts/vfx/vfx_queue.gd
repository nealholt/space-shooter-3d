...
func face_and_play(loc:Vector2, to_face:Vector2) -> int:
	var index := next
	# If the current effect is playing then we're maxed
	# out. Prefer to skip than to interrupt
	if vfx_players[next].is_playing():
...
func play_at_angle(loc:Vector2, angle:float) -> int:
	var index := next
	# If the current effect is playing then we're maxed
	# out. Prefer to skip than to interrupt
	if vfx_players[next].is_playing():
...
func play_remote_transform(remote_mover:Node2D) -> int:
	var index := next
	# If the current effect is playing then we're maxed
	# out. Prefer to skip than to interrupt
	if vfx_players[next].is_playing():
