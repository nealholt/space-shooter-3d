...
## Handles [method create_2d_audio_at_location()] and
...
## each unique sound effect, create a Node2D scene for this
...
## [method create_2d_audio_at_location()] and [method create_audio()]
...
func stop_all(type:int, use_2d:=false) -> void:
	if use_2d:
		type = type + 1
	error_check(type)
	sound_effect_dict[type].stop_all()

func stop(type:int, index:int=0, use_2d:=false) -> void:
	if use_2d:
		type = type + 1
	error_check(type)
	sound_effect_dict[type].stop(index)

func get_volume(type:int, index:int=0, use_2d:=false) -> float:
	if use_2d:
		type = type + 1
	error_check(type)
	return sound_effect_dict[type].get_volume(index)

func set_volume(type:int, new_volume:float, index:int=0, use_2d:=false) -> void:
	if use_2d:
		type = type + 1
	error_check(type)
	sound_effect_dict[type].set_volume(new_volume, index)

func set_volume_percent(type:int, percent:float, index:int=0, use_2d:=false) -> void:
	if use_2d:
		type = type + 1
	error_check(type)
